import React, { useState } from 'react'

export default function Sidebar() {
  const [collapsed, setCollapsed] = useState(true)
  const [activeItem, setActiveItem] = useState('Dashboard')

  return (
    <aside className={`${collapsed ? 'w-20' : 'w-64'} h-screen bg-white dark:bg-gray-800 shadow-md transition-all duration-300 z-20 relative`}>
      <div className="flex flex-col h-full">
        {/* Botón para colapsar/expandir */}
        <button
          onClick={() => setCollapsed(!collapsed)}
          className="absolute right-12 top-20 bg-blue-600 text-white p-1 rounded-full shadow-lg transform translate-x-full hover:bg-blue-700 transition-colors"
        >
          {collapsed ? (
            <svg className="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
              <path d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" />
            </svg>
          ) : (
            <svg className="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
              <path d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clipRule="evenodd" />
            </svg>
          )}
        </button>

        {/* Logo */}
        <div className="flex items-center justify-center py-5 border-b border-gray-200 dark:border-gray-700">
          {/* Logo para modo claro - visible solo en light */}
          <img src="/Logo.png" alt="LogoLight" className="h-12 w-auto block dark:hidden" />
          {/* Logo para modo oscuro - visible solo en dark */}
          <img src="/LogoWhite.png" alt="LogoLight" className="h-12 w-auto hidden dark:block" />          
        </div>

        {/* Menú de navegación */}
        <nav className="flex-1 overflow-y-auto py-7">
          <ul className="space-y-2 px-3">
            {[
              { name: 'Dashboard', icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
              { name: 'Mascotas', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z' },
              { name: 'Adopciones', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01' },
              { name: 'Donaciones', icon: 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z' },
              { name: 'Usuarios', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z' },
              { name: 'Configuración', icon: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z' },
            ].map((item, index) => (
              <li key={index} className={collapsed ? "flex justify-center" : ""}>
                <a
                  href={item.link}
                  onClick={() => setActiveItem(item.name)}
                  className={`flex items-center p-2 rounded-lg ${activeItem === item.name ? 'bg-blue-100 text-blue-600 dark:bg-blue-900 dark:text-blue-300' : 'text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700'}`}>
                  <svg
                    className="h-6 w-6"
                    fill="none"
                    stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={item.icon} />
                  </svg>
                  {!collapsed && <span className="ml-3">{item.name}</span>}
                </a>
              </li>
            ))}
          </ul>
        </nav>

        {/* Perfil de usuario */}
        <div className="border-t border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center">
            <div className="flex-shrink-0">
              <img
                className="h-10 w-10 rounded-full"
                src="data:image/jpeg;base64,"
                alt="Usuario"
              />
            </div>
            {!collapsed && (
              <div className="ml-3">
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Admin</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">admin@adoptafacil.com</p>
              </div>
            )}
          </div>
        </div>
      </div>
    </aside>
  )
}